...
<div className="card member-card shadow-sm hover:shadow-md border-1  rounded-sm w-full !max-w-[310px] border bg-white mx-auto flex flex-col overflow-hidden">
    ...
    <div className="card-details w-full transition-all border-t-2 bg-white flex flex-col items-start justify-between py-8 pl-3 flex-1 h-auto">
      ...
      <ul className="mt-4">
          <li className="inline-block mr-4"><Link href={email?`mailto:${email}`:"#"} target={email&&'_blank'} scroll={false} className="text-red-500"><MdEmail className='text-2xl'/></Link></li>
          <li className="inline-block mr-4"><Link href={facebook} target={(facebook!=='#')?'_blank':""} scroll={false} className="text-[#1877F2]"><IoLogoFacebook className='text-2xl'/></Link></li>
          <li className="inline-block mr-4"><Link href={linkedIn} target={(linkedIn!=='#')?'_blank':''} scroll={false} className="text-[#0077B5]"><IoLogoLinkedin className='text-2xl'/></Link></li>
        </ul>
    </div>
</div>
